...
export const adminUpdateUser = createServerFn({ method: "POST" })
  ...
  .inputValidator((d: { user_id: string; full_name?: string; email?: string; phone?: string; company?: string; role?: "admin" | "client" }) => d)
...
export const adminCreateUser = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((d: { email: string; password: string; full_name?: string; role: "admin" | "client" }) => d)
  .handler(async ({ data, context }) => {
    await assertAdmin(context);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    await supabaseAdmin.from("profiles").upsert({ id: created.user.id, email: data.email, full_name: data.full_name ?? null });
