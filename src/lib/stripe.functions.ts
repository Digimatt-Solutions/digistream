...
export const createStripeCheckout = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    body.append(
      "success_url",
      `${data.origin}/app/subscription?stripe=success&pkg=${pkg.id}&sid={CHECKOUT_SESSION_ID}`,
    );
    ...
    if (pkg.description)
      body.append("line_items[0][price_data][product_data][description]", pkg.description);
