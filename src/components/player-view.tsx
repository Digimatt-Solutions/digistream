...
export function PlayerView({ standalone = false }: { standalone?: boolean }) {
  ...
  const filtered = useMemo(() => {
    ...
    return content.filter((c: any) =>
      `${c.title} ${c.artist ?? ""} ${c.category ?? ""}`.toLowerCase().includes(s),
    );
  }, [content, search]);
  ...
  const grouped = useMemo(() => {
    ...
    filtered.forEach((c: any) => {
      (g[c.content_type ?? "video"] ||= []).push(c);
    });
    const ordered: [string, any[]][] = [];
    TYPE_ORDER.forEach((t) => {
      if (g[t]?.length) ordered.push([t, g[t]]);
    });
    Object.keys(g).forEach((k) => {
      if (!TYPE_ORDER.includes(k as never)) ordered.push([k, g[k]]);
    });
    return ordered;
  }, [filtered]);
    ...
    return (
      <Card className="mx-auto max-w-lg rounded-2xl p-8 text-center shadow-[var(--shadow-card)]">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Subscribe to a package to unlock streaming content.
        </p>
        <Button asChild className="mt-4">
          <a href="/app/subscription">View plans</a>
        </Button>
      </Card>
    );
  }
  ...
  if (!content || content.length === 0) {
    return (
      <Card className="mx-auto max-w-lg rounded-2xl p-8 text-center shadow-[var(--shadow-card)]">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Your subscription is active but no content has been added.
        </p>
      </Card>
    );
  }
        ...
        <div className="relative overflow-hidden rounded-3xl shadow-[var(--shadow-elevated)]">
          ...
          <div className="absolute inset-0 flex items-end p-5 md:p-12">
            <div className="max-w-xl text-white">
              ...
              <p className="mt-2 line-clamp-2 text-xs text-white/85 sm:text-sm md:text-base">
                {hero.description}
              </p>
              <div className="mt-4 flex gap-2">
                <Button
                  ...
                  className="rounded-xl font-semibold text-white hover:opacity-90"
                >
            ...
            <div className="flex items-start justify-between gap-3">
              <div>
                <div className="text-[11px] font-semibold uppercase tracking-widest text-white/60">
                  {selected.content_type} • {selected.category}
                </div>

                <h2 className="mt-1 text-xl font-bold sm:text-2xl">{selected.title}</h2>

                {selected.artist && (
                  <div className="text-sm text-white/80">by {selected.artist}</div>
                )}
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            {filtered.length} items available on your <b>{sub.packages?.name}</b> plan
          </p>
        </div>
