...
function KpiCard({
  ...
  return (
    <Card className="group relative overflow-hidden rounded-2xl border-border p-5 shadow-[var(--shadow-card)] transition hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)]">
      <div
        className={`absolute -right-8 -top-8 h-24 w-24 rounded-full bg-gradient-to-br ${tints[tint]} opacity-70 blur-2xl`}
      />
      <div className="relative flex items-start justify-between">
        ...
        <div
          className={`flex h-11 w-11 items-center justify-center rounded-xl bg-gradient-to-br ${tints[tint]}`}
        >
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [users, content, packages, subs, logs] = await Promise.all([
        ...
        supabase
          .from("subscriptions")
          .select("id, package_id, packages(name, price_monthly)")
          .eq("status", "active"),
        supabase
          .from("activity_logs")
      ...
      (content.data ?? []).forEach((c: any) =>
        cats.set(c.content_type ?? "video", (cats.get(c.content_type ?? "video") ?? 0) + 1),
      );
      const planCounts = new Map<string, number>();
      (subs.data ?? []).forEach((s: any) => {
        const n = s.packages?.name ?? "-";
        planCounts.set(n, (planCounts.get(n) ?? 0) + 1);
      });
      return {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 sm:gap-4 xl:grid-cols-6">
        ...
        <KpiCard label="Active Subs" value={data?.activeSubs ?? "-"} icon={Users} tint="warning" />
        <KpiCard
          label="MRR"
        ...
        <Card className="rounded-2xl p-6 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {(data?.categories ?? []).map((c, i) => (
              <span
                key={c.name}
                className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-2.5 py-1 text-xs font-medium capitalize"
              >
                <span
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      const [sub, allContent, allowed] = await Promise.all([
        supabase
          .from("subscriptions")
          .select("id, status, expires_at, started_at, packages(name, max_streams, price_monthly)")
          .eq("user_id", user!.id)
          .order("created_at", { ascending: false })
          .limit(1)
          .maybeSingle(),
        ...
        supabase
          .from("content")
          .select("id, title, thumbnail_url, content_type, category")
          .eq("active", true)
          .order("created_at", { ascending: false })
          .limit(6),
      ]);
      ...
      (allContent.data ?? []).forEach((c: any) =>
        typeCounts.set(
          ...
          (typeCounts.get(c.content_type ?? "video") ?? 0) + 1,
        ),
      );
      return {
        sub: sub.data as any,
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Play, brand and manage your subscription in one place.
        </p>
      </div>

      {/* Hero */}
      <Card
        className="relative overflow-hidden rounded-3xl border-0 p-8 shadow-[var(--shadow-elevated)]"
        ...
        <div className="relative z-10 grid gap-6 text-primary-foreground md:grid-cols-[1fr_auto] md:items-center">
          <div>
            ...
            <p className="mt-1 max-w-md opacity-95">
              {active
                ? "Open the player to stream what's included in your package."
                : "Subscribe to a plan to unlock streaming."}
            </p>
          ...
          {active && (
            <div className="rounded-2xl bg-black/25 border p-5 backdrop-blur">
              <div className="text-xs text-white uppercase tracking-widest opacity-80">
                Renews / expires
              </div>
              ...
              {daysLeft !== null && (
                <div className="mt-1 text-sm opacity-90">{daysLeft} days remaining</div>
              )}
              <div className="mt-3 text-xs opacity-80">
                {ksh(Number(data?.sub?.packages?.price_monthly ?? 0))} / mo · up to{" "}
                {data?.sub?.packages?.max_streams ?? "-"} streams
              </div>
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
        ...
        <KpiCard
          label="Videos"
          value={data?.typeCounts?.find((t) => t.name === "video")?.value ?? 0}
          icon={Video}
          tint="info"
        />
        <KpiCard
          label="Songs"
          value={data?.typeCounts?.find((t) => t.name === "song")?.value ?? 0}
          icon={Music2}
          tint="success"
        />
        <KpiCard
          label="Mixes"
          value={data?.typeCounts?.find((t) => t.name === "mix")?.value ?? 0}
          icon={Radio}
          tint="warning"
        />
      </div>
      ...
      <Card className="rounded-2xl p-6 shadow-[var(--shadow-card)]">
        <div className="flex items-center justify-between">
          ...
          <Link to="/app/watch" className="text-xs font-semibold text-primary hover:underline">
            Browse all →
          </Link>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {(data?.recent ?? []).map((c: any) => (
            <Link
              key={c.id}
              to="/app/watch"
              className="group overflow-hidden rounded-xl border border-border transition hover:-translate-y-0.5 hover:shadow-md"
            >
