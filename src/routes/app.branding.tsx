...
function BrandingPage() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("client_brands").select("*").eq("user_id", user!.id).maybeSingle()).data,
    enabled: !!user,
  });
  ...
  const save = async () => {
    ...
    const { error } = await supabase
      .from("client_brands")
      .upsert({ user_id: user!.id, ...form, updated_at: new Date().toISOString() } as never);
    setBusy(false);
    ...
    await supabase
      .from("activity_logs")
      .insert({ user_id: user!.id, action: "brand.updated", entity: "client_brand" } as never);
  };
    ...
    <div className="mx-auto max-w-6xl space-y-6" style={{ fontFamily: form.font_family }}>
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          Fine-tune every touchpoint of your player experience.
        </p>
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_1fr]">
        <Card className="rounded-2xl p-6 shadow-[var(--shadow-card)]">
          <Tabs defaultValue="identity">
            ...
            <TabsContent value="identity" className="mt-4 space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="Great content, anywhere"
                />
              </div>
              <div>
                <Label>Logo</Label>
                <div className="mt-1.5 flex items-center gap-3">
                  {form.logo_url ? (
                    <img src={form.logo_url} alt="" className="h-14 w-auto rounded border" />
                  ) : (
            ...
            <TabsContent value="colors" className="mt-4 space-y-4">
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, background_type: v })}
                >
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(v) => setForm({ ...form, theme_mode: v })}
                >
                  <SelectTrigger className="mt-1.5">
                    <SelectValue />
                  </SelectTrigger>
            ...
            <TabsContent value="type" className="mt-4 space-y-4">
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder="Displayed on the player home."
                />
              </div>
            </TabsContent>
            ...
            <TabsContent value="player" className="mt-4 space-y-4">
              <div>
                ...
                <Input
                  className="mt-1.5"
                  value={form.player_title}
                  onChange={(e) => setForm({ ...form, player_title: e.target.value })}
              ...
              <div>
                ...
                <div className="mt-1.5 flex items-center gap-3">
                  {form.default_thumbnail_url ? (
                    <img
                      src={form.default_thumbnail_url}
                      alt=""
                ...
                <Input
                  ...
                  onChange={(e) => setForm({ ...form, default_thumbnail_url: e.target.value })}
                  placeholder="Or paste image URL"
                />
              </div>
            </TabsContent>
          ...
          <Button className="mt-6 w-full" onClick={save} disabled={busy}>
            {busy && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Save brand
          </Button>
        </Card>
        ...
        >
          <div className="p-4 text-xs uppercase tracking-wide text-muted-foreground">
            Live preview
          </div>
          <div className="p-8 text-white" style={{ background: bg }}>
            <div className="flex items-center gap-3">
              {form.logo_url ? (
                <img src={form.logo_url} alt="" className="h-10 w-auto" />
              ) : (
            ...
            {form.welcome_message && (
              <p className="mt-4 text-sm opacity-95">{form.welcome_message}</p>
            )}
            ...
            <div className="mt-4 flex items-center gap-2">
              ...
              <button className="rounded-lg border border-white/40 bg-white/10 px-3 py-1.5 text-sm">
                More info
              </button>
            </div>
          </div>
