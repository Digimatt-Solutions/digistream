...
function ActivityPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (search)
        q = q.or(`action.ilike.%${search}%,entity.ilike.%${search}%,page.ilike.%${search}%`);
      ...
      <div className="flex flex-col justify-between gap-3 md:flex-row md:items-end">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Every user action, page visit, and admin change - fully paginated.
          </p>
        </div>
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Search action, page, entity..."
          ...
          <>
                ...
                <TableBody>
                  {rows.map((l: any) => (
                    <TableRow key={l.id} className="hover:bg-muted/30">
                      ...
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-xs font-bold text-primary">
                            {(l.profile?.full_name ?? l.profile?.email ?? "?")
                              .slice(0, 2)
                              .toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <div className="truncate text-sm font-medium">
                              {l.profile?.full_name ?? l.profile?.email ?? "System"}
                            </div>
                      ...
                      <TableCell>
                        <span
                          className={`inline-flex rounded-full px-2 py-0.5 text-[11px] font-semibold ${ACTION_TINTS[l.action] ?? "bg-secondary text-secondary-foreground"}`}
                        >
                          {l.action}
                        </span>
                      </TableCell>
            ...
            <div className="flex flex-col items-center justify-between gap-3 border-t border-border bg-muted/20 px-4 py-3 sm:flex-row">
              <div className="text-xs text-muted-foreground">
                Showing <b>{page * PAGE_SIZE + 1}</b>-
                <b>{Math.min((page + 1) * PAGE_SIZE, total)}</b> of <b>{total}</b> records
              </div>
              <div className="flex items-center gap-2">
                <Button
                  variant="outline"
