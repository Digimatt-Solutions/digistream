...
import { Loader2, Globe, Smartphone, Monitor, Tablet, Chrome, TrendingUp, Users } from "lucide-react";
import {
...
const COLORS = ["#f97316", "#0ea5e9", "#22c55e", "#eab308", "#a855f7", "#ef4444", "#14b8a6", "#f43f5e"];
...
function SettingsPage() {
  ...
  const changePassword = async () => {
    ...
    await supabase.from("activity_logs").insert({ user_id: user!.id, action: "password.updated", entity: "auth" } as never);
  };
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">Security, preferences{role === "admin" ? " and platform traffic" : ""}.</p>
      </div>
...
function TrafficAnalytics() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => (await supabase.from("activity_logs").select("action, country, device, browser, os, created_at").order("created_at", { ascending: false }).limit(2000)).data ?? [],
  });
  ...
  return (
    <Card className="rounded-2xl p-6 shadow-[var(--shadow-card)]">
      <div className="mb-4 flex items-center justify-between">
        <div>
          <h3 className="flex items-center gap-2 text-lg font-bold"><TrendingUp className="h-4 w-4 text-primary" />Platform traffic</h3>
          <p className="text-xs text-muted-foreground">Aggregated from real page-view events - last 2,000 records.</p>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card className="rounded-xl p-4 lg:col-span-2">
          <div className="text-xs font-semibold uppercase tracking-widest text-muted-foreground">Visits - 14d</div>
          <div className="mt-3 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={stats.trend}>
                ...
                <Area type="monotone" dataKey="visits" stroke="#f97316" strokeWidth={2.5} fill="url(#gv)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card className="rounded-xl p-4">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground"><Smartphone className="h-3 w-3" />Devices</div>
          <MiniPie data={stats.byDevice} />
        </Card>

        <Card className="rounded-xl p-4">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground"><Globe className="h-3 w-3" />Top countries</div>
        ...
        <Card className="rounded-xl p-4">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground"><Chrome className="h-3 w-3" />Browsers</div>
          <MiniPie data={stats.byBrowser} />
        </Card>

        <Card className="rounded-xl p-4">
          <div className="mb-2 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-widest text-muted-foreground"><Monitor className="h-3 w-3" />Operating systems</div>
...
function MiniPie({ data }: { data: { name: string; value: number }[] }) {
  if (!data.length) return <div className="flex h-56 items-center justify-center text-xs text-muted-foreground">No data yet</div>;
  return (
    <div className="h-56">
      <ResponsiveContainer width="100%" height="100%">
        <PieChart>
