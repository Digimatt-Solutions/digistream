...
function BillingPage() {
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const mrr = active.reduce((s: number, r: any) => s + Number(r.packages?.price_monthly ?? 0), 0);
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const rev = rows
          .filter((r: any) => r.started_at && new Date(r.started_at).getMonth() === d.getMonth() && new Date(r.started_at).getFullYear() === d.getFullYear())
          .reduce((s: number, r: any) => s + Number(r.amount_paid ?? r.packages?.price_monthly ?? 0), 0);
  ...
  const stats = useMemo(() => ([
    { label: "MRR", value: ksh(data?.mrr ?? 0), icon: TrendingUp, tint: "text-success bg-success/10" },
    { label: "Total collected", value: ksh(data?.totalCollected ?? 0), icon: Wallet, tint: "text-primary bg-primary/10" },
    { label: "Active subs", value: data?.active.length ?? 0, icon: Users, tint: "text-info bg-info/10" },
    { label: "Total records", value: data?.subs.length ?? 0, icon: CreditCard, tint: "text-warning-foreground bg-warning/20" },
  ]), [data]);
        ...
        {stats.map((s) => (
          <Card key={s.label} className="rounded-2xl p-5 shadow-[var(--shadow-card)]">
            <div className="flex items-start justify-between">
              ...
              <div className={`flex h-10 w-10 items-center justify-center rounded-xl ${s.tint}`}><s.icon className="h-5 w-5" /></div>
            </div>
          </Card>
        ))}
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="mt-4 h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data?.months ?? []}>
                ...
                <Tooltip formatter={(v: any) => ksh(Number(v))} contentStyle={{ borderRadius: 12 }} />
        ...
        <Card className="rounded-2xl p-6 shadow-[var(--shadow-card)]">
          ...
          <div className="mt-2 h-56">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data?.planShare ?? []} dataKey="value" nameKey="name" innerRadius={45} outerRadius={80} paddingAngle={3}>
                  {(data?.planShare ?? []).map((_, i) => <Cell key={i} fill={COLORS[i % COLORS.length]} />)}
                </Pie>
        ...
        {isLoading ? (
          ...
          <Table>
            ...
            <TableBody>
              ...
              {(data?.subs ?? []).length === 0 && (
                <TableRow><TableCell colSpan={7} className="py-8 text-center text-muted-foreground">No subscriptions yet.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        )}
