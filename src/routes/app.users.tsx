  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const [profiles, roles, subs] = await Promise.all([
        ...
        supabase
          .from("subscriptions")
          .select("user_id, status, packages(name)")
          .eq("status", "active"),
      ]);
              ...
              <TableBody>
                {filtered.map((u: any) => (
                  <TableRow key={u.id} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      {u.plan ?? <span className="text-muted-foreground">No plan</span>}
                    </TableCell>
      ...
      <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently remove <b>{deleting?.email}</b> and all their subscription
